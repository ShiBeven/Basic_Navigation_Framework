...
public:
  ...
  // Sets the symmetric integral (sum-error) clamp: integral is limited to
  // [-limit, +limit]. Must be > 0; non-positive values are ignored.
  void setSumLimit(double limit);
  ~PID();
...
private:
  ...
  double pre_error_;
  double integral_;
  double integral_limit_ = 1.0;  // symmetric anti-windup clamp, overridable via setSumLimit()
};

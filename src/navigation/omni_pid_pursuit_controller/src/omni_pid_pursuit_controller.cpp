  ...
  std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // Apply the configured integral (sum-error) clamp; previously min_max_sum_error was
  // read from parameters but never forwarded, leaving the PID stuck at a hardcoded ±1.
  move_pid_->setSumLimit(min_max_sum_error_);
  heading_pid_->setSumLimit(min_max_sum_error_);
}

  ...
  const sensor_msgs::msg::Joy::SharedPtr joy_msg, const std::string & which_map)
{
  ...
  try {
    auto transform = tf_buffer_->lookupTransform(global_frame_, robot_base_frame_, tf2::TimePointZero);
    tf2::doTransform(gimbal_pose, goal.pose, transform);
  } catch (tf2::TransformException & ex) {
    RCLCPP_WARN(
      this->get_logger(), "Failed to transform goal pose from %s to %s: %s",

...
void LoamInterfaceNode::pointCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  // Wait until the odometry callback has initialized the transform, otherwise the
  // cloud would be transformed with an identity (i.e. wrong) transform.
  if (!base_frame_to_lidar_initialized_) {
    RCLCPP_WARN_ONCE(
      this->get_logger(),
      "Received point cloud before odom->lidar_odom transform is initialized; dropping "

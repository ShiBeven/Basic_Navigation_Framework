...
private:
  ...
  // NOTE: tf2::Transform's default ctor does NOT initialize to identity; initialize
  // explicitly so an early pointCloudCallback (before the odom callback sets it) does
  // not transform with garbage memory.
  tf2::Transform tf_odom_to_lidar_odom_ = tf2::Transform::getIdentity();
};

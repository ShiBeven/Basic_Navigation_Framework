...
void SmallGicpRelocalizationNode::loadGlobalMap(const std::string & file_name)
{
  ...
  const int max_retries = 30;   // max ~30 seconds
  ...
  while (retry_count < max_retries) {
    try {
      ...
      RCLCPP_WARN(this->get_logger(),
        "TF lookup failed (attempt %d/%d): %s. Retrying...",
        retry_count, max_retries, ex.what());
      if (retry_count >= max_retries) {
        RCLCPP_ERROR(this->get_logger(),
          "TF lookup for %s -> %s failed after %d attempts. "
          "Skipping global map transform — map will stay in original frame.",
          base_frame_.c_str(), lidar_frame_.c_str(), max_retries);
        return;  // keep global_map_ in its original frame, skip transform
      }

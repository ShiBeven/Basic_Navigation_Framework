...
private:
  ...
  // Velocity is derived by finite-differencing consecutive poses. Kept as members
  // (not function-local statics) so multiple node instances don't share history and
  // state is reset when the node is reconstructed.
  bool has_previous_odom_ = false;
  tf2::Transform previous_odom_transform_;
  std::chrono::steady_clock::time_point previous_odom_time_;
};

...
// frame id of the published terrain map (parameterized for cross-project reuse)
std::string mapFrameId = "odom";
...
int main(int argc, char **argv) {
  ...
  nh->declare_parameter<std::string>("mapFrameId", mapFrameId);
  ...
  nh->get_parameter("mapFrameId", mapFrameId);
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      terrainCloud2.header.frame_id = mapFrameId;

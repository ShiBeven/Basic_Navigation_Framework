...
void Preprocess::process_cut_frame_pcl2(
  ...
  deque<double> & time_lidar, const int required_frame_num, int scan_count)
{
  // Initialize time_unit_scale to ensure correct timestamp conversion
  switch (time_unit) {
    case SEC:
      time_unit_scale = 1.e3f;
      break;
  ...
  if (lidar_type == VELO16) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature =
        (pl_orig.points[i].timestamp - rclcpp::Time(msg->header.stamp).seconds()) * time_unit_scale;  // curvature unit: ms
